    ...
    watch: {
        catalogPath(path) {
            ...
            if (path && path.length === 3) {
                this.$http({
                    url: `/product/attrgroup/list/${path[path.length - 1]}`,
                    method: "get",
    ...
    methods: {
        init(id) {
            ...
            this.$nextTick(() => {
                ...
                if (this.dataForm.attrId) {
                    this.$http({
                        url: `/product/attr/info/${this.dataForm.attrId}`,
                        method: "get",
        ...
        dataFormSubmit() {
            this.$refs["dataForm"].validate(valid => {
                if (valid) {
                    this.$http({
                        url: `/product/attr/${!this.dataForm.attrId ? "save" : "update"}`,
                        method: this.dataForm.attrId ? 'put' : 'post',
